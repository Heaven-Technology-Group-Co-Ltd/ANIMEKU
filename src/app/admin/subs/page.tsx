...
export default function SubsEditorPage() {
  ...
  const containerId = "subs-editor-player";
          ...
          onReady: (e: YTPlayerEvent) => {
            ...
            timerRef.current = window.setInterval(() => {
              try {
                ...
                if (c !== undefined && !isNaN(c)) setCurrent(c);
                if (d !== undefined && !isNaN(d)) setDuration(d);
              } catch {}
    ...
    <div className="min-h-screen bg-[#06060a] text-white">
      <div className="mx-auto max-w-[1280px] px-4 py-6">
        <h1 className="text-2xl font-black">
          จูนซับให้ตรง — กดๆ จบ <span className="text-[#ff3b82]"> subs editor</span>
        </h1>
          ...
          <div>
            <div className="flex gap-2 mb-3">
              <input
                ...
                placeholder="YouTube videoId เช่น k4xGqY5IDBE"
                className="flex-1 rounded-full bg-white/10 border border-white/20 px-4 py-2 text-sm outline-none focus:border-[#ff3b82]"
              />
              <button
                onClick={handleLoad}
                className="rounded-full bg-[#ff3b82] px-6 py-2 text-sm font-bold hover:bg-[#ff4f8f]"
              >
                โหลดคลิป
              </button>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              >
                + เพิ่มซับที่ {current.toFixed(1)}s
              </button>
              <button
                onClick={sortCues}
                className="rounded-full bg-white/10 border border-white/20 px-4 py-2 text-xs"
              >
                เรียงตามเวลา
              </button>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-[#12121a] p-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleCopy} className="rounded-full bg-[#ff3b82] px-4 py-1.5 text-xs font-bold">
                  {copied ? "ก็อปแล้ว ✓" : "ก็อปโค้ด"}
                </button>
              </div>
              <pre className="mt-3 max-h-[220px] overflow-auto rounded-xl bg-black p-3 text-xs leading-5 text-zinc-300 border border-white/10 whitespace-pre-wrap">
                {exportCode}
              </pre>
                ...
                {cues.map((c, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border p-2.5 cursor-pointer transition ${isActive ? "border-[#ff3b82] bg-[#ff3b82]/15" : isSelected ? "border-white/30 bg-white/5" : "border-white/10 bg-black/40 hover:bg-white/5"}`}
                    >
                      <div className="flex items-center gap-1.5">
                        <span className="text-[11px] font-mono text-zinc-500">#{idx + 1}</span>
                        {isActive && (
                          <span className="text-[10px] bg-[#ff3b82] text-white px-1.5 py-0.5 rounded-full font-bold animate-pulse">
                            ON AIR
                          </span>
                        )}
                      </div>
                      <div className="mt-2 grid grid-cols-[1fr_1fr_auto] gap-1.5">
                        <div>
                          ...
                          <div className="flex gap-1">
                            <input
                              ...
                              value={c.start}
                              onChange={(e) =>
                                setCues((prev) => prev.map((x, i) => (i === idx ? { ...x, start: Number(e.target.value) } : x)))
                              }
                              className="w-full rounded-lg bg-white/10 border border-white/15 px-2 py-1 text-xs font-mono"
                            />
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className="rounded-lg bg-emerald-600 px-2 text-xs font-bold"
                            >
                              S
                            </button>
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex gap-1">
                            <input
                              ...
                              value={c.end}
                              onChange={(e) =>
                                setCues((prev) => prev.map((x, i) => (i === idx ? { ...x, end: Number(e.target.value) } : x)))
                              }
                              className="w-full rounded-lg bg-white/10 border border-white/15 px-2 py-1 text-xs font-mono"
                            />
                            <button
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <button
                            ...
                            className="rounded-lg bg-red-500/20 text-red-300 px-2 py-1 text-xs"
                          >
                            ลบ
                          </button>
                        </div>
                      ...
                      <textarea
                        value={c.text}
                        onChange={(e) => setCues((prev) => prev.map((x, i) => (i === idx ? { ...x, text: e.target.value } : x)))}
                        rows={2}
                        className="mt-2 w-full rounded-lg bg-white/10 border border-white/15 px-2 py-1.5 text-xs"
                        placeholder="พิมพ์ซับ..."
                      />
                      <div className="mt-1 text-[10px] font-mono text-zinc-600">
                        {(c.end - c.start).toFixed(1)}s • {c.start.toFixed(1)} → {c.end.toFixed(1)}
                      </div>
                    </div>
                  );
                })}
                {cues.length === 0 && (
                  <p className="text-xs text-zinc-500 text-center py-8">ยังไม่มีซับ — กด &quot;เพิ่มซับ&quot; แล้วพิมพ์ได้เลย</p>
                )}
              </div>
            </div>

            <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 text-xs leading-5 text-amber-200">
              <b>วิธีใช้ให้ตรงสุด (10 วิจบ):</b>
              <br />
              1. กดเล่น → ฟังประโยคแรก
              <br />
              2. กดหยุดตรงเริ่มพูด → กด <b>S</b> ที่ cue นั้น
              <br />
              3. เลื่อนไปท้ายประโยค → กด <b>E</b>
              <br />
